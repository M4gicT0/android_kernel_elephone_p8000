					...
					unsigned newpagemovable)
{
	...
	while (pfn < end || !list_empty(&cc->migratepages)) {
		...
		if (list_empty(&cc->migratepages)) {
			...
			ret = ret < 0 ? ret : -EBUSY; if (tries == 100000000) pr_alert("%s %d: max tries: %d\n", __func__, __LINE__, tries);
			break;
		} else {
			msleep(1);
		...
		nr_reclaimed = reclaim_clean_pages_from_list(cc->zone,
							&cc->migratepages); //pr_alert("%s %d: start: %lu, pfn: %lu, end: %lu\n", __func__, __LINE__, start, pfn, end);printk("%s %d: nr_reclaimed %lu cc->nr_migratepages %lu\n", __func__, __LINE__, nr_reclaimed, cc->nr_migratepages);
		cc->nr_migratepages -= nr_reclaimed;

// 20150125 0953, thlin modified start
		if (newpagemovable) {
			ret = migrate_pages(&cc->migratepages, alloc_migrate_target,
				    0, MIGRATE_SYNC, MR_CMA); //if (ret) pr_alert("%s %d: start: %lu, pfn: %lu, end: %lu ret %d\n", __func__, __LINE__, start, pfn, end, ret);
		}
		else {
...
// 20150125 0953, thlin modified end
	...
	if (ret < 0) {
		putback_movable_pages(&cc->migratepages); pr_alert("%s %d: ret %d\n", __func__, __LINE__, ret);
		return ret;
	} //pr_alert("%s %d: ret %d\n", __func__, __LINE__, 0);
	return 0;
}
	...
	for (round = 0; round < 1000; round++) {
		...
		for (__i = 0; __i < blocksize; __i++) {
			...
			while (pfn < end || !list_empty(&ncc->migratepages)) {
				if (fatal_signal_pending(current)) {
					aret = -EINTR; pr_alert("%s %d: round [%d] ssc[%d]\n", __func__, __LINE__, round, __i);
					goto out;
				}
				...
				nr_reclaimed = reclaim_clean_pages_from_list(ncc->zone,
									&ncc->migratepages); //pr_alert("%s %d: start: %lu, pfn: %lu, end: %lu\n", __func__, __LINE__, start, pfn, end);printk("%s %d: nr_reclaimed %lu cc->nr_migratepages %lu\n", __func__, __LINE__, nr_reclaimed, cc->nr_migratepages);
				ncc->nr_migratepages -= nr_reclaimed;

				// 20150125 0953, thlin modified start
				if (newpagemovable) {
					pscc[__i].ret = migrate_pages(&ncc->migratepages, alloc_migrate_target,
					    0, MIGRATE_SYNC, MR_CMA); //if (ret) pr_alert("%s %d: start: %lu, pfn: %lu, end: %lu ret %d\n", __func__, __LINE__, start, pfn, end, ret);
				} else {
					pscc[__i].ret = migrate_pages(&ncc->migratepages, alloc_migrate_target_notmovable,
					    0, MIGRATE_SYNC, MR_CMA); //if (ret) pr_alert("%s %d: start: %lu, pfn: %lu, end: %lu ret %d\n", __func__, __LINE__, start, pfn, end, ret);
				}
		...
		for (__i = 0; __i < blocksize; __i++) {
			pr_alert("%s %d: round [%d] end: scc[%d] %lu %lu %lu fin %d\n", __func__, __LINE__, round, __i, pscc[__i].start, pscc[__i].pfn, pscc[__i].end, pscc[__i].finish);
		}

		if (_finCnt == blocksize) break;
...
out:
	...
	if (aret < 0) {
		for (__i = 0; __i < blocksize; __i++) {
			putback_movable_pages(&pscc[__i].cc.migratepages); //pr_alert("%s %d: ret %d\n", __func__, __LINE__, ret);
		}
